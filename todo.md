...
- [x] Replace direct shared mock-data usage with customer-scoped, API-ready domain contracts and repository adapters across the portal.
- [x] Define the authenticated backend routes, database ownership model, query keys, error contracts, and migration path needed for plug-in API integration.
- [x] Migrate shipment, invoice, payment, profile, addresses, recipients, notifications, support, returns, pickup, delivery, and draft workflows to the API-ready data boundary.
- [x] Add contract and repository tests, integration documentation, and a mock-adapter removal checklist for live API activation.
- [x] Ensure every customer page reads data only through typed, customer-scoped query hooks and never directly from mock fixtures or browser records.
...
- [x] Ensure completed public-tracking timeline connectors remain visibly Cargo Yellow through the in-transit stage.
