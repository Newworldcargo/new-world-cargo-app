...
export default function Home() {
  ...
  const inTransit =
    ...
    shipments.find(shipment => shipment.id !== arriving?.id) ??
    arriving;
  const outstandingInvoice = invoices.find(
    invoice => invoice.status === "unpaid"
  );
  const walletBalance = wallet
    ? `${wallet.currency} ${(wallet.availableBalance.amountMinor / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    : null;
  ...
  const track = () => {
    ...
    const matchingShipment = shipments.find(
      shipment =>
        shipment.trackingNumber.toLowerCase() === tracking.trim().toLowerCase()
    );
    navigate(
      matchingShipment
        ? `/shipments/${matchingShipment.id}`
        : `/track?number=${encodeURIComponent(tracking.trim().toUpperCase())}`
    );
  };
  ...
  const primaryActions = [
    {
      ...
      onClick: () =>
        document
          .getElementById("tracking-rail")
          ?.scrollIntoView({ behavior: "smooth", block: "center" }),
      accent: "bg-cargo-yellow text-ink border-cargo-yellow",
    },
    {
      label: "Pay balance",
      detail: outstandingInvoice
        ? `${outstandingInvoice.amount} due for cargo`
        : "Review your cargo payments",
      icon: WalletCards,
    ...
    <div className="mx-auto w-full min-w-0 max-w-6xl overflow-x-hidden">
      ...
      <div className="mt-6 lg:grid lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-8">
        <main className="min-w-0 lg:border-r lg:border-ink/10 lg:pr-8">
          ...
          <section className="mt-6 min-w-0">
            <div className="mb-3 flex items-end justify-between gap-4">
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs font-bold text-cargo-yellow"
              >
                All bookings <ChevronRight className="size-4" />
              </button>
            </div>
                  ...
                  >
                    ...
                    <div className="mt-6 flex items-end justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-1 text-[10px] font-medium leading-4 opacity-65">
                          {action.detail}
                        </p>
                      </div>
          ...
          <section className="mt-9 min-w-0">
            <div data-sidebar-section="/shipments">
              ...
              {inTransit ? (
                ...
                <div className="rounded-[24px] border border-dashed border-ink/15 bg-white p-6 text-sm text-ink/60">
                  No open bookings are linked to this customer account.
                </div>
              )}
            </div>
          </section>
          ...
          <section
            data-sidebar-section="/send"
            className="mt-10 rounded-[28px] border border-ink/10 bg-[#f7f8fb] p-5 sm:p-7"
          >
            <div className="grid gap-6 sm:grid-cols-[1fr_auto] sm:items-end">
              <div>
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-cargo-yellow">
                  <Sparkles className="size-4" /> More with New World Cargo
                </div>
                <h2 className="mt-3 font-heading text-2xl font-bold tracking-tight text-foreground">
                  Need to send something new?
                </h2>
                <p className="mt-2 max-w-xl text-sm leading-6 text-white/55">
                  Create a local delivery or arrange a China shipment when you
                  are ready.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-2xl border border-ink/10 bg-white px-4 py-3 text-xs font-bold text-foreground transition hover:border-cargo-yellow/70"
                >
                  <ReceiptText className="size-4" /> Get a quote
                </button>
                <button
                  onClick={() => navigate("/send")}
                  className="inline-flex items-center gap-2 rounded-2xl border border-cargo-yellow bg-cargo-yellow px-4 py-3 text-xs font-bold text-ink transition hover:brightness-[1.02]"
                >
                  <PackagePlus className="size-4" /> New shipment
                </button>
              </div>
            </div>
          </section>
          ...
          <section className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-ink/10 bg-white px-4 py-3">
            <div className="flex items-center gap-3">
              <div className="grid size-8 place-items-center rounded-xl bg-cargo-yellow/25 text-ink">
                <ShieldCheck className="size-4" />
              </div>
              <span className="text-xs font-semibold text-white/70">
                Need help with a booking or delivery?
              </span>
            </div>
            <button
              onClick={() => navigate("/support")}
              className="text-xs font-bold text-cargo-yellow"
            >
              Talk to support
            </button>
          </section>
        </main>
        <aside className="mt-6 min-w-0 space-y-6 lg:mt-0 lg:pl-0">
          <section
            data-sidebar-section="/invoices"
            className="rounded-[28px] border border-ink/10 bg-[#f7f8fb] p-5 sm:p-6"
          >
            <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/45">
                ...
                <p className="mt-2 break-words text-sm leading-6 text-white/55">
                  {outstandingInvoice
                    ? `${outstandingInvoice.amount} due for your ${outstandingInvoice.route} order.`
                    : "Your assigned cargo wallet is ready for payments and refunds."}
                </p>
              </div>
              <div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-cargo-yellow/15 text-cargo-yellow">
          ...
          <section
            id="tracking-rail"
            className="min-w-0 overflow-hidden rounded-[28px] border border-ink/10 bg-[#f7f8fb] p-5 sm:p-6"
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <MapPin className="size-5 text-cargo-yellow" />
            </div>
            <div className="mt-5 rounded-2xl border border-ink/10 bg-white p-2">
              <div className="flex items-center gap-2 px-3">
                <Search className="size-5 text-ink/45" />
                <input
                  value={tracking}
                  onChange={event => setTracking(event.target.value)}
                  onKeyDown={event => event.key === "Enter" && track()}
                  placeholder="Enter tracking number"
                  className="h-12 min-w-0 flex-1 bg-transparent text-sm font-semibold text-ink outline-none placeholder:text-ink/40"
                  aria-label="Tracking number"
                />
                <button
                  onClick={track}
                  className="grid size-10 place-items-center rounded-full bg-ink text-white transition hover:bg-ink/80"
            ...
            <div className="mt-3 flex items-center justify-between px-1 text-[11px] text-white/55">
              <span>Try NWC48291ZM</span>
              <button
                onClick={() => navigate("/track")}
                className="flex items-center gap-1.5 font-semibold text-cargo-yellow"
              >
