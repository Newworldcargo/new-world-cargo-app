...
export default function Drafts() {
  ...
  return <div className="mx-auto max-w-3xl text-ink">
    ...
    {draft ? <div className="mt-6 rounded-[26px] border border-ink/10 bg-white p-5"><p className="text-sm font-bold">{String((draft.payload.form as { recipient?: string } | undefined)?.recipient || "New cargo request")}</p><p className="mt-1 text-xs text-ink/55">{String((draft.payload.form as { pickup?: string } | undefined)?.pickup || "Pickup not selected")} · {String((draft.payload.transport || "air") === "sea" ? "Sea cargo" : "Air cargo")}</p><p className="mt-4 text-xs text-ink/45">Saved on the server. Drafts are not submitted until the request workflow completes.</p><div className="mt-5 flex flex-wrap gap-2"><Button onClick={() => navigate(`/send?draft=${encodeURIComponent(draft.id)}`)} className="rounded-xl bg-cargo-yellow font-bold text-ink"><Play className="mr-2 size-4" /> Resume draft</Button><Button variant="outline" disabled={draftMutations.remove.isPending} onClick={() => draftMutations.remove.mutate({ id: draft.id, revision: draft.revision })} className="rounded-xl font-bold"><Trash2 className="mr-2 size-4" /> {draftMutations.remove.isPending ? "Deleting…" : "Delete draft"}</Button></div></div> : <div className="mt-6 rounded-[26px] border border-dashed border-ink/20 bg-[#f7f8fb] p-8 text-center"><EmptyState title="No saved drafts" detail="Start a shipment and choose Save as Draft to keep it on your account." /><Button onClick={() => navigate("/send")} className="mt-5 rounded-xl bg-cargo-yellow font-bold text-ink">Start a shipment</Button></div>}
  </div>;
}
