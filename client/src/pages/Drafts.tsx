...
function resumePath(draft: { id: string; payload: Record<string, unknown> }) {
  const service = String(draft.payload.service ?? "");
  const stage = typeof draft.payload.stage === "string" ? draft.payload.stage : "route";
  if (["local", "intercity", "import", "custom"].includes(service) && typeof draft.payload.wizardDraft === "object") {
    return `/send/${service}/${stage}?draft=${encodeURIComponent(draft.id)}`;
  }
  return `/send?draft=${encodeURIComponent(draft.id)}`;
}
