...
import { clearVerificationPending, extractRecoveryIdentifier, hasVerificationPending, isStrongPassword, markVerificationPending, type RegistrationFieldErrors, type RegistrationForm, validateRegistration } from "@/lib/auth-workflow";
...
export function Login() {
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setError(""); if (!identifier || !password) { setError("Enter your email or phone number and password."); return; } setLoading(true); const result = await login(identifier, password); setLoading(false); if (!result.ok) { if (result.reason === "unverified") markVerificationPending(); setError(result.reason === "disabled" ? "We could not open your customer account. Use account recovery so our team can check it." : result.reason === "unverified" ? "This account is not verified yet. Verify your account to continue." : result.reason === "service" ? "We couldn't sign you in right now. Please try again." : "That email/phone number and password combination isn't correct."); return; } if (result.user && !result.user.verified) { markVerificationPending(); navigate("/verify"); return; } clearVerificationPending(); navigate("/"); }
  ...
  return <AuthLayout title="Welcome back" eyebrow="Customer access" variant="customer-login"><form onSubmit={submit} className="grid gap-4"><p className="-mt-3 text-sm leading-6 text-ink/60">Sign in to manage your shipments and tracking updates.</p>{error && <AuthError>{error}{error.includes("verified") && <Link href="/verify" className="ml-1 font-bold underline">Verify account</Link>}</AuthError>}<Field label="Email or phone" value={identifier} onChange={setIdentifier} autoComplete="email" placeholder="you@example.com" /><PasswordField label="Password" value={password} onChange={setPassword} /><div className="flex items-center justify-end"><Link href="/forgot-password" className="text-sm font-semibold text-[#b8860b] underline underline-offset-2">Forgot password?</Link></div><Button className="h-12 rounded-xl bg-cargo-yellow font-bold text-ink hover:bg-cargo-yellow/90" disabled={loading}>{loading && <Busy />}Sign in</Button><div className="flex items-center gap-3 text-xs text-ink/35"><span className="h-px flex-1 bg-ink/10" />or<span className="h-px flex-1 bg-ink/10" /></div><GoogleAuthButton onClick={google} loading={googleLoading} /><Link href="/recover-account" className="text-center text-sm font-semibold text-ink underline underline-offset-2">Already shipped with us but cannot sign in?</Link><p className="text-center text-sm text-ink/55">Don't have an account? <Link href="/register" className="font-bold text-ink underline underline-offset-2">Create account</Link></p></form></AuthLayout>;
}

export function Register() {
  const [, navigate] = useLocation(); const { register, googleLogin } = useAuth(); const [form, setForm] = useState<RegistrationForm>({ firstName: "", lastName: "", email: "", phone: "", password: "", confirm: "" }); const [accepted, setAccepted] = useState(false); const [fieldErrors, setFieldErrors] = useState<RegistrationFieldErrors>({}); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  const set = (key: keyof RegistrationForm) => (value: string) => { setForm(prev => ({ ...prev, [key]: value })); setFieldErrors(prev => ({ ...prev, [key]: undefined })); };
  async function submit(e: React.FormEvent) { e.preventDefault(); setError(""); const localErrors = validateRegistration(form, accepted); setFieldErrors(localErrors); if (Object.keys(localErrors).length) { setError("Please correct the highlighted fields."); return; } setLoading(true); const result = await register({ firstName: form.firstName.trim(), lastName: form.lastName.trim(), email: form.email.trim().toLowerCase(), phone: form.phone.trim(), password: form.password }); setLoading(false); if (!result.ok) { const backendErrors: RegistrationFieldErrors = {}; for (const key of ["firstName", "lastName", "email", "phone", "password"] as const) { const message = result.fieldErrors?.[key]?.[0]; if (message) backendErrors[key] = message; } setFieldErrors(backendErrors); setError(result.reason === "existing" ? "An account already exists with this email. Sign in or recover your existing account." : result.reason === "validation" ? result.message || "Please correct the highlighted fields." : "We couldn't create your account right now. Please try again."); return; } markVerificationPending(); navigate("/verify"); }
  return <AuthLayout title="Create your account"><form onSubmit={submit} className="grid gap-3" noValidate><p className="-mt-3 mb-2 text-sm text-ink/55">Keep every shipment within reach.</p>{error && <AuthError>{error}</AuthError>}<div className="grid gap-3 sm:grid-cols-2"><Field label="First name" value={form.firstName} onChange={set("firstName")} autoComplete="given-name" error={fieldErrors.firstName} /><Field label="Last name (optional)" value={form.lastName} onChange={set("lastName")} autoComplete="family-name" error={fieldErrors.lastName} /></div><Field label="Email address" value={form.email} onChange={set("email")} type="email" autoComplete="email" error={fieldErrors.email} /><Field label="Phone number" value={form.phone} onChange={set("phone")} type="tel" autoComplete="tel" error={fieldErrors.phone} /><PasswordField label="Password" value={form.password} onChange={set("password")} autoComplete="new-password" error={fieldErrors.password} /><PasswordRequirements password={form.password} /><PasswordField label="Confirm password" value={form.confirm} onChange={set("confirm")} autoComplete="new-password" error={fieldErrors.confirm} /><label className="flex items-start gap-2 py-1 text-xs leading-5 text-ink/60"><input type="checkbox" checked={accepted} onChange={e => { setAccepted(e.target.checked); setFieldErrors(prev => ({ ...prev, accepted: undefined })); }} className="mt-1 accent-[#FFC83D]" />I agree to the <Link href="/settings/legal/terms" className="font-semibold text-ink underline">Terms</Link> and <Link href="/settings/legal/privacy" className="font-semibold text-ink underline">Privacy Policy</Link>.</label>{fieldErrors.accepted && <span className="text-xs font-medium text-red-700" role="alert">{fieldErrors.accepted}</span>}<Button className="h-12 rounded-xl bg-cargo-yellow font-bold text-ink hover:bg-cargo-yellow/90" disabled={loading}>{loading && <Busy />}Create account</Button><GoogleAuthButton onClick={() => { googleLogin().then(() => navigate("/auth/complete-profile")); }} /><Link href="/recover-account" className="text-center text-sm font-semibold text-ink underline underline-offset-2">Find or recover an existing customer account</Link><p className="text-center text-sm text-ink/55">Already have an account? <Link href="/login" className="font-bold text-ink underline">Sign in</Link></p></form></AuthLayout>;
}
...
export function Verify() {
  ...
  async function resend() { if (resending) return; setResending(true); setError(""); try { const result = await resendVerification(); if (result.ok) { setCode(""); setResent(true); } } catch { setError("We could not request another code. Please try again shortly."); } finally { setResending(false); } }
  return <AuthLayout title="Verify your account"><form onSubmit={submit} className="grid gap-5"><p className="-mt-3 text-sm leading-6 text-ink/55">Enter the 6-digit code from your verification message. Request a new code if you have not received one.</p>{success ? <AuthSuccess title="Account verified">You’re ready to continue.</AuthSuccess> : <>{error && <AuthError>{error}</AuthError>}<div className="grid justify-items-center gap-3"><OtpInput value={code} onChange={setCode} /><button type="button" onClick={resend} disabled={resending} className="inline-flex items-center gap-2 text-sm font-semibold text-ink disabled:opacity-60"><RefreshCw className={`size-4 ${resending ? "animate-spin" : ""}`} />{resending ? "Requesting code" : resent ? "Request another code" : "Resend code"}</button></div><Button className="h-12 rounded-xl bg-cargo-yellow font-bold text-ink" disabled={loading}>{loading && <Busy />}Verify</Button><Link href="/login" className="text-center text-sm font-semibold text-ink/55">Return to sign in</Link></>}</form></AuthLayout>;
}
...
function PasswordRecovery({ initialIdentifier = "" }: { initialIdentifier?: string }) {
  ...
  if (!codeSent) return <AuthLayout title="Reset your password"><form onSubmit={requestCode} className="grid gap-5"><p className="-mt-3 text-sm leading-6 text-ink/55">Enter your customer email address or phone number. If the details match an eligible account, we will email a six-digit code to its saved email address.</p>{error && <AuthError>{error}</AuthError>}<Field label="Email or phone" value={identifier} onChange={setIdentifier} autoComplete="username" placeholder="you@example.com or +260…" /><Button disabled={loading} className="h-12 rounded-xl bg-cargo-yellow font-bold text-ink">{loading && <Busy />}Send verification code</Button><Link href="/recover-account" className="text-center text-sm font-semibold text-ink underline">Cannot access your email or never signed in?</Link><BackToSignIn /></form></AuthLayout>;

  return <AuthLayout title="Enter verification code"><form onSubmit={savePassword} className="grid gap-5"><p className="-mt-3 text-sm leading-6 text-ink/55">If an eligible account matches <strong>{identifier.trim()}</strong>, check its saved email address for a six-digit code. The code expires in 10 minutes and can only be used once.</p>{error && <AuthError>{error}</AuthError>}<div className="grid justify-items-center gap-3"><OtpInput value={code} onChange={value => setCode(value.replace(/\D/g, "").slice(0, 6))} /><button type="button" onClick={resendCode} disabled={resending} className="inline-flex items-center gap-2 text-sm font-semibold text-ink disabled:opacity-60"><RefreshCw className={`size-4 ${resending ? "animate-spin" : ""}`} />{resending ? "Sending code" : "Resend code"}</button></div><PasswordField label="New password" value={password} onChange={setPassword} autoComplete="new-password" /><PasswordRequirements password={password} /><PasswordField label="Confirm new password" value={confirmation} onChange={setConfirmation} autoComplete="new-password" /><Button disabled={loading} className="h-12 rounded-xl bg-cargo-yellow font-bold text-ink">{loading && <Busy />}Verify code and update password</Button><button type="button" onClick={() => { setCodeSent(false); setCode(""); setError(""); }} className="text-sm font-semibold text-ink/55 underline underline-offset-2">Use a different email or phone</button><Link href="/recover-account" className="text-center text-sm font-semibold text-ink underline">Cannot access your email or never signed in?</Link><BackToSignIn /></form></AuthLayout>;
}
