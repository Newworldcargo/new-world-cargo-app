...
const groups: { label: string; links: SettingsLink[] }[] = [
  { label: "Your details", links: [
    { title: "Account details", detail: "Profile, contact, and account information", href: "/settings/account", icon: CircleUserRound, accent: "bg-route-lavender/15 text-route-lavender" },
    ...
    { title: "Saved recipients", detail: "People you send to often", href: "/settings/recipients", icon: UsersRound, accent: "bg-mint/30 text-emerald-800" },
  ] },
  { label: "Preferences & payments", links: [
    { title: "Notifications", detail: "Shipment updates and delivery alerts", href: "/settings/notifications", icon: Bell, accent: "bg-route-lavender/15 text-route-lavender" },
    ...
    { title: "Security", detail: "Password and verified contact details", href: "/settings/security", icon: ShieldCheck, accent: "bg-mint/30 text-emerald-800" },
    { title: "Receipts & billing", detail: "Invoices, payments, and receipts", href: "/settings/billing", icon: WalletCards, accent: "bg-route-lavender/15 text-route-lavender" },
  ] },
];
...
export default function Settings() {
  ...
  return <div className="mx-auto max-w-3xl text-ink">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-cargo-yellow">Customer settings</p><h1 className="mt-2 font-heading text-3xl font-extrabold tracking-tight">Settings</h1><p className="mt-2 max-w-xl text-sm leading-6 text-ink/55">Manage your details, delivery preferences, payment options, and account protection one thing at a time.</p></div><div className="inline-flex w-fit items-center gap-2 rounded-2xl bg-route-lavender/12 px-3 py-2 text-xs font-bold text-route-lavender"><span className="size-2 rounded-full bg-route-lavender" /> Account ready</div></div>
    <div className="relative mt-7 overflow-hidden rounded-[26px] border border-ink/10 bg-[#f7f8fb] p-4 sm:p-5"><div className="absolute -bottom-7 right-8 size-16 rounded-2xl border-[10px] border-cargo-yellow/25" /><div className="relative flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-cargo-yellow text-ink"><ReceiptText className="size-5" /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-ink/45">Account control rail</p><p className="mt-1 text-sm font-extrabold">Details <span className="text-ink/35">·</span> delivery <span className="text-ink/35">·</span> payment <span className="text-ink/35">·</span> security</p></div><div className="hidden items-center gap-2 sm:flex"><span className="size-2 rounded-full bg-mint" /><span className="h-px w-10 bg-mint" /><span className="size-3 rounded-full border-2 border-route-lavender bg-white shadow-[0_0_0_4px_rgba(129,120,255,0.14)]" /><span className="h-px w-10 border-t border-dashed border-ink/20" /><span className="size-2 rounded-full bg-ink/25" /></div></div></div>
