...
export default function Shipments() {
  ...
  return <div className="mx-auto max-w-6xl"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-heading text-3xl font-extrabold tracking-tight sm:text-4xl">Shipments</h1><p className="mt-2 text-sm text-white/45">Every package, one calm view.</p></div><button onClick={() => navigate("/send")} className="flex items-center justify-center gap-2 rounded-2xl bg-cargo-yellow px-4 py-3 text-sm font-bold text-ink transition hover:brightness-105"><span className="text-lg leading-none">+</span> Send a package</button></div>
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-2">{filtered.length ? filtered.map((shipment) => <div key={shipment.id}><ShipmentCard shipment={shipment} onOpen={() => navigate(`/shipments/${shipment.id}`)} /></div>) : <div className="col-span-full rounded-[28px] border border-dashed border-white/15 p-10 text-center"><Filter className="mx-auto size-6 text-white/30" /><p className="mt-3 font-heading font-bold">No shipments match that search</p><p className="mt-1 text-sm text-white/40">Try another tracking number or clear the filter.</p></div>}</div>
    <div className="mt-8 flex items-center gap-2 text-xs text-white/30"><div className="size-1.5 rounded-full bg-cargo-yellow" /> Tracking updates refresh automatically in your customer account.</div>
  </div>;
}
