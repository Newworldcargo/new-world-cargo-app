...
import { PageSkeleton, MutationProgress } from "./components/loading-skeleton";
...
function CustomerRouter() {
  ...
  if (pageState === "loading") return <PageSkeleton path={location} />;
...
function PublicTrackingRoute() {
  ...
  if (pageState === "loading") return <PageSkeleton variant="detail" />;
...
function RoutedApp() {
  ...
  const isPublic = ["/recover-account", "/login", "/register", "/verify", "/forgot-password", "/reset-password", "/password/reset", "/auth/complete-profile", "/session-expired", "/shipments/tracking", "/track"].some(path => location.startsWith(path)) || isLegacyPublicTracking || location.startsWith("/settings/legal");
  ...
  if (sessionLoading) return <AppPreloader />;
  ...
  return <AppShell><Suspense fallback={<PageSkeleton path={location} />}><CustomerRouter /></Suspense></AppShell>;
}

export default function App() { return <ThemeProvider defaultTheme="light"><TooltipProvider><Toaster theme="light" position="top-center" closeButton richColors /><ErrorBoundary><AuthProvider><OfflineBanner /><MutationProgress /><Suspense fallback={<AppPreloader label="Loading New World Cargo…" />}><RoutedApp /></Suspense></AuthProvider></ErrorBoundary></TooltipProvider></ThemeProvider>; }
