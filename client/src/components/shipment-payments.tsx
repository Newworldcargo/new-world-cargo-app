...
export function ShipmentPayments({ shipmentId, reference, cancelled }: { shipmentId: string; reference: string; cancelled: boolean }) {
  const { user } = useAuth();
  ...
  return <section className="border-t border-ink/15 py-5 text-ink" aria-labelledby="shipment-payments-heading">
    ...
    {isLoading ? <p className="mt-3 text-sm" role="status">Loading payments...</p> : isError || !data ? <div className="mt-3" role="alert"><p>We couldn't load payment details.</p><button className="mt-2 font-semibold underline" onClick={() => refetch()}>Try again</button></div> : <>
