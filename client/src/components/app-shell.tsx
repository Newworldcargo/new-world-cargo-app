      ...
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        <aside className="sticky top-0 hidden h-screen w-[236px] shrink-0 flex-col border-r border-ink/10 bg-white px-5 py-7 lg:flex">
          ...
          <div className="mt-14 flex flex-1 flex-col gap-1">
            ...
            <button onClick={() => toast("Pickup locations are ready to browse.")} className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left text-sm font-semibold text-white/55 transition hover:bg-white/6 hover:text-white">
              <Truck className="size-[18px]" strokeWidth={1.8} /> Pickup locations
            </button>
          </div>
          <button onClick={() => setSupportOpen(true)} className="mt-auto flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-left transition hover:border-cargo-yellow/50 hover:bg-white/8">
            <div className="grid size-9 place-items-center rounded-xl bg-mint/15 text-mint"><Sparkles className="size-4" /></div>
            <div><p className="text-xs font-bold">Need a hand?</p><p className="mt-0.5 text-[11px] text-white/45">Support is online</p></div>
          </button>
        </aside>
