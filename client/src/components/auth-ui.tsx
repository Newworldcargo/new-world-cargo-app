...
export function AuthLayout({ children, title, eyebrow = "New World Cargo" }: { children: React.ReactNode; title?: string; eyebrow?: string }) {
  return <div className="nwc-light min-h-screen bg-background px-4 py-6 text-foreground sm:px-6 sm:py-10"><div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-md flex-col"><div className="mb-8 flex items-center justify-between"><Link href="/login" aria-label="New World Cargo home"><BrandMark compact /></Link><span className="text-xs font-semibold text-ink/45">{eyebrow}</span></div><main className="my-auto rounded-[30px] border border-ink/10 bg-white p-5 shadow-[0_20px_60px_rgba(1,38,66,0.08)] sm:p-8">{title && <div className="mb-7"><p className="text-xs font-bold uppercase tracking-[0.18em] text-cargo-yellow">Customer access</p><h1 className="mt-2 font-heading text-3xl font-bold text-ink">{title}</h1></div>}{children}</main><p className="mt-6 text-center text-[11px] leading-5 text-ink/45">By continuing, you agree to our <Link href="/settings/legal/terms" className="font-semibold text-ink underline underline-offset-2">Terms</Link> and <Link href="/settings/legal/privacy" className="font-semibold text-ink underline underline-offset-2">Privacy Policy</Link>.</p></div></div>;
}
