...
export function PaymentModal({ open, amount, reference, onClose, onSuccess }: PaymentModalProps) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-end justify-center bg-ink/40 p-3 backdrop-blur-sm sm:items-center" role="dialog" aria-modal="true" aria-label="Make a payment">
    <div className="w-full max-w-lg overflow-hidden rounded-[30px] border border-ink/10 bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-ink/8 px-5 py-4 sm:px-7"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-ink/45">Secure payment</p><p className="mt-1 text-sm font-semibold text-ink">{reference}</p></div><button onClick={onClose} className="grid size-9 place-items-center rounded-full border border-ink/10 text-ink/50 transition hover:bg-ink/5 hover:text-ink" aria-label="Close payment"><X className="size-4" /></button></div>
      <div className="p-5 sm:p-7"><div className="flex items-center justify-between rounded-2xl bg-[#f7f8fa] p-4"><div><p className="text-xs font-semibold text-ink/45">Amount to pay</p><p className="mt-1 font-heading text-2xl font-extrabold text-ink">{amount}</p></div><LockKeyhole className="size-5 text-ink/35" /></div>
