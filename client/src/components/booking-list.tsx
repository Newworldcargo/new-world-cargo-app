...
export function BookingList({ query, filter }: { query: string; filter: "all" | "active" | "delivered" }) {
  ...
  return <section className="mt-8" aria-labelledby="bookings-heading">
    ...
    {isLoading ? <p role="status" className="text-sm text-ink/65">Loading bookings...</p> : isError ? <div role="alert"><p>We couldn't load your bookings.</p><button onClick={() => refetch()} className="mt-2 rounded-lg border border-ink/20 px-4 py-2 font-semibold">Try again</button></div> : bookings.length ? <ul className="divide-y divide-ink/10 border-y border-ink/10">
