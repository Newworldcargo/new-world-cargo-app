  ...
  return (
    <button onClick={onOpen} className="group w-full text-left" aria-label={`Open shipment ${shipment.trackingNumber}`}>
      <div className={`relative overflow-hidden rounded-[28px] p-5 transition duration-200 group-hover:-translate-y-1 group-hover:shadow-[0_18px_40px_rgba(1,38,66,0.12)] sm:p-6 ${tone}`}>
        ...
        <div className="relative mt-7 grid grid-cols-[1fr_auto_1fr] items-end gap-3"><div><p className={`text-[10px] font-bold uppercase tracking-[0.16em] ${muted}`}>From</p><p className="mt-1 text-sm font-bold">{shipment.origin.split(",")[0]}</p></div><div className="mb-1 flex items-center gap-1.5"><span className={`size-2 rounded-full ${isAir ? "bg-ink" : "bg-cargo-yellow"}`} /><span className={`h-px w-10 ${routeLine}`} /><span className={`size-2 rounded-full border-2 ${routeEnd}`} /></div><div className="text-right"><p className={`text-[10px] font-bold uppercase tracking-[0.16em] ${muted}`}>To</p><p className="mt-1 text-sm font-bold">{shipment.destination.split(",")[0]}</p></div></div>
