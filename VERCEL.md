...
The current Express server bundle remains a local/static SPA runtime and is not the BFF. The actual Cargo API must be separately deployed and provide the BFF service-authentication and session-exchange contract described in [`SERVER_SIDE_GATEWAY_ARCHITECTURE.md`](./SERVER_SIDE_GATEWAY_ARCHITECTURE.md). For the production release, set `NWC_BACKEND_ORIGIN=https://api.newworldcargo.com` and `NWC_BACKEND_API_PREFIX=/api` in the server environment. Configure these values only in Vercel Preview and Production: `NWC_BACKEND_ORIGIN`, `NWC_BACKEND_API_PREFIX`, `NWC_BFF_SERVICE_TOKEN`, `NWC_BFF_SHARED_SECRET`, `NWC_BFF_ALLOWED_ORIGIN`, and `NWC_BFF_TIMEOUT_MS`. Never add the backend host, service token, database credentials, payment secrets, or signing keys to `VITE_*` variables. Preview deployments should use a staging API hostname, not the production API hostname.
